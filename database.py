...
class dbworker:
	...
	def create_profile(self, telegram_id, telegram_username, name, description, city, photo, sex, age):
		'''Создаём анкету'''
		with self.connection:
			return self.cursor.execute("INSERT INTO profile_list (telegram_id, telegram_username, name, description, city, photo, sex, age) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", (telegram_id, telegram_username, name, description, city, photo, sex, age))
	...
	def search_profile(self, age, sex):
		...
		try:
			if str(sex) == 'мужчина':
				...
				sex_search = 'мужчина'
			with self.connection:
				return self.cursor.execute("SELECT telegram_id FROM profile_list WHERE sex = ? ORDER BY age DESC", (sex_search, )).fetchall()
	...
	def edit_profile_status(self, user_id, num):
		'''изменение статуса'''
		with self.connection:
			return self.cursor.execute('UPDATE users SET search_id = ? WHERE telegram_id = ?',(str(int(num) + 1), user_id))
	...
	'''Мои тестовые записи'''
	def send_photo(self, tg_id):
		with self.connection:
			return self.cursor.execute('SELECT photo FROM profile_list WHERE telegram_id = ?', (tg_id, )).fetchall()
