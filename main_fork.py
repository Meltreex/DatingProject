...
async def seach_profile_step2(message: types.Message, state: FSMContext):
	...
	db.set_city_search(str(user_data['search_profile_city']), str(message.from_user.id)) # Обновляет город в таблице users - city search
...
async def seach_profile_step3(message: types.Message, state: FSMContext):
	...
	try:
		if str(message.text) == '👍':
			...
			city = str(db.get_info(5302034245)[5]).title()
...
async def admin(msg: types.Message):
	if msg.from_user.id in config.ADMIN_LIST:
		...
		await msg.answer('Панель админа\n\nКнопка Add Bot - для создания бота отправь мне фото и подпись к ней в формате(текст-текст-текст)')
	else:
...
async def cmd_bot_admin(msg: types.Message):
	if msg.from_user.id in config.ADMIN_LIST:
		await send_log(msg)
		await bot.send_message(msg.from_user.id, 'Для создания бота отправь мне фото и подпись к ней в формате(текст-текст-текст)')
...
async def add_bot_desc(msg: types.Message, state: FSMContext):
	if msg.from_user.id in config.ADMIN_LIST:
		...
		tg_id = mss_caption[0]
		tg_username = mss_caption[1]
		name = mss_caption[2]
		description = mss_caption[3]
		city = mss_caption[4]
		sex = mss_caption[5]
		age = mss_caption[6]
...
async def send_log(message: types.Message):
	await bot.send_message(-4199373261,f'ID - {str(message.from_user.id)}\nusername - {str(message.from_user.username)}\nmessage - {str(message.text)}')
